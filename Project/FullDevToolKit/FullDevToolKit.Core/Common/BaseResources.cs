...
namespace FullDevToolKit.Common
{
    ...
    public class LocalizationResource
    {
        ...
        private Dictionary<string, string> _resource = null;

        public void Set(List<LocalizationTextResult> resource, string lang)
        {
            _resource = new Dictionary<string, string>();

            foreach (LocalizationTextResult r in resource)
            {
                if (r.LanguageID.ToString() == lang)
                {

                    if (!_resource.ContainsKey(r.Name))
                    {
                        _resource.Add(r.Name, r.Text);
                    }
                }
            }
        }
    ...
    public class ConfigurationResource
    {
        ...
        public void Set(List<ConfigsResult> resource)
        {
            _resource = new Dictionary<string, string>();

            foreach (ConfigsResult r in resource)
            {
                if (!_resource.ContainsKey(r.ConfigName))
                {
                    _resource.Add(r.ConfigName, r.ConfigValue);
                }
